...
> 最後更新：2026-08-03（第一百五十六輪，**5 個📖手冊按鈕改成一律從第一頁開啟**）：使用者反映每個手冊點下按鈕後應該預設從第一頁開始，不要直接跳章節。拿掉上一輪加的 `#page=N` 錨點，[`RosterView.vue`](src/views/roster/RosterView.vue)／[`OfficersView.vue`](src/views/club/OfficersView.vue)／[`AccountManagementView.vue`](src/views/admin/AccountManagementView.vue)／[`ActivityListView.vue`](src/views/activities/ActivityListView.vue)／[`AttendanceMonthlyView.vue`](src/views/meetings/AttendanceMonthlyView.vue) 的 `manual-url` 都改成純檔案路徑（例如 `/manual/roster-officers-guide.pdf`，不再帶 `#page=3`）。`vue-tsc --noEmit` 通過，純字串改動未另外開瀏覽器驗證。
