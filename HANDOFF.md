...
> **待使用者：** ① 到 Supabase Dashboard SQL Editor 手動執行 `074_activities_club_only_category_fix.sql`（回填既有外洩資料＋加 CHECK constraint），跑完後可以請旭光社帳號實際確認看不到和平社的社內活動；② 部署前端後找一個社友帳號測試新增「社內活動」，確認「招募對象」欄位變成固定說明文字（不能再選全地區）；③ 找一場例會實際點「儲存出席記錄」，確認跳出「已儲存出席記錄」的綠色提示，並測「← 返回」按鈕行為符合預期。
