...
**【第七十二輪】RotarySSO — 待使用者完成以下事項，程式碼已完成但還沒 push**：

1. **到 RotarySSO 後台註冊 OIDC client**（`https://rotarysso.vercel.app/admin/oauth-clients` → 新增客戶端）：應用程式名稱建議「扶輪3481地區管理平台」；Redirect URIs 填 `http://localhost:5174/auth/sso/callback` 跟 `https://d3481clubmanagementsystem.pages.dev/auth/sso/callback`（正式站網域取自 `invite-user` Edge Function 現有的 `SITE_URL` 常數，如果有另外綁自訂網域要另外告知）；客戶端類型選「機密（confidential）」；Scope 填 `openid profile rotary`；「內部應用程式」不勾；「允許的帳號類型」不勾。建立後把彈出的 `client_id`／`client_secret` 提供給 Claude，會直接用 `supabase secrets set` 寫進 Edge Function 環境變數。
2. **執行 [`051_rotarysso.sql`](supabase/migrations/051_rotarysso.sql)**（`user_profiles` 新增 `sso_sub`/`sso_account_type`/`sso_rotary_club`/`sso_rotary_district` 四欄＋改寫 `handle_new_user()` trigger）。
3. **部署 `sso-login` Edge Function**＋設定 `ROTARYSSO_CLIENT_ID`／`ROTARYSSO_CLIENT_SECRET` 兩個 secret（拿到 client_id/secret 後 Claude 可代勞）。
4. **前端環境變數**：Cloudflare Pages 專案設定加 `VITE_ROTARYSSO_CLIENT_ID`（值＝上面拿到的 client_id，這個不是密碼，可以放前端環境變數）。
5. **實測至少一輪完整登入流程**：用 RotarySSO 帳號登入（建議先用 `account_type='管理者'` 的帳號測，預期直接拿到 `district_role='admin'`，不會卡在待審頁），確認能落地成正常 session；再用一個一般帳號測「待審核」流程，到「帳號管理」頁確認能看到新欄位並成功指派社別。
6. **確認沒問題後，再考慮要不要去 Supabase Dashboard → Authentication → Providers 停用 Email/Password 登入**——這步 Claude不會主動做，因為關掉之後還沒轉 SSO 的既有社友（尤其是用手機號碼登入、RotarySSO 沒有手機 claim 沒辦法自動比對到的一般社友）會直接登不進去，要等確認狀況後再手動關閉。
