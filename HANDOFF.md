...
**【第七十三輪】RotarySSO — 只剩最後複查，前面的部署步驟都已完成**：

1. ~~到 RotarySSO 後台註冊 OIDC client~~ 已完成 ✅（client_id `F1z2S113VsnSJE9Xzn8cu`，機密類型，兩個 redirect URI 都登記好）
2. ~~執行 051_rotarysso.sql~~ 已完成 ✅
3. ~~部署 sso-login Edge Function + 設定 secrets~~ 已完成 ✅（過程中發現要另外去 Dashboard 把這支函式的「Enforce JWT Verification」關掉，已關閉）
4. ~~Cloudflare Pages 設定 VITE_ROTARYSSO_CLIENT_ID~~ 已完成 ✅（記得改環境變數後要重新觸發部署，`VITE_*` 是 build time 寫死的）
5. **【還沒做完】重新走一次完整登入流程**：第七十三輪修掉 `SsoCallbackView.vue` 的 `verifyOtp` 400 錯誤（`git push` 後 Cloudflare 會自動重新部署），麻煩推上去、部署完成後再測一次 https://d3481clubmanagementsystem.pages.dev/login →「用扶輪帳號登入」，確認這次能不能真正落地登入成功（用 `account_type='管理者'` 的帳號測，預期直接拿到 `district_role='admin'`、不卡在待審頁；再用一個一般帳號測「待審核」流程，到「帳號管理」頁確認能看到新欄位並成功指派社別）。
6. **確認沒問題後，再考慮要不要去 Supabase Dashboard → Authentication → Providers 停用 Email/Password 登入**——這步 Claude 不會主動做，因為關掉之後還沒轉 SSO 的既有社友（尤其是用手機號碼登入、RotarySSO 沒有手機 claim 沒辦法自動比對到的一般社友）會直接登不進去，要等確認狀況後再手動關閉。
