...
2. 部署兩支 Edge Function（都會用到 `denomailer` 這個 Deno SMTP 套件透過 Gmail SMTP 寄信，不需要額外設定 secret，帳密是存在資料庫裡、每社各自輸入的）。**這輪兩支都改了內文亂碼的修復，不管之前有沒有部署過都要重新貼一次最新內容**：
