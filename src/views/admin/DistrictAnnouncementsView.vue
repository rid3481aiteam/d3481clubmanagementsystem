...
import type { DistrictAnnouncement } from '@/types'
...
const modalTitle = computed(() => editingId.value ? '編輯地區公告' : '新增地區公告')
...
function openEdit(item: DistrictAnnouncement) {
...
async function save() {
  ...
  const { error } = editingId.value
    ? await announcements.updateDistrictAnnouncement(editingId.value, payload)
    : await announcements.createDistrictAnnouncement({ ...payload, created_by: auth.user?.id ?? null })
...
async function remove(item: DistrictAnnouncement) {
  if (!confirm(`刪除「${item.title}」？`)) return
  const { error } = await announcements.deleteDistrictAnnouncement(item.id)
  if (error) alert(error.message)
}

onMounted(() => {
  announcements.fetchDistrictForAdmin()
})
</script>
  ...
  <div class="page">
    ...
    <div class="tw">
      <table>
        ...
        <tbody>
          <tr v-for="item in announcements.adminDistrictAnnouncements" :key="item.id">
            <td>
